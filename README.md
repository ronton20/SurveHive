...
- **Main menu & scene flow** (Phase 4B): a generated `MainMenu` scene (the boot scene in build settings) in the pixel kit — home (Play / Hive Upgrades / Settings / Quit), world select (Beehive playable; Garden and Woods shown locked; a difficulty dropdown seam fixed to Normal for now), and the **Hive Upgrades shop** listing all six permanent upgrades with live rank/cost/balance and buy buttons that grey out when unaffordable. Death and victory screens gained **RETRY** and **HIVE** buttons (reload run / back to menu); tap-anywhere restart was removed in favor of the buttons (R still restarts from the keyboard).
