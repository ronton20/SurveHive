...
- **Pause & settings** (Phase 4C): ESC or the HUD pause button freezes the run completely (every spawner/cooldown runs on scaled time) and opens resume / settings / abandon — abandoning banks the run's honey and returns to the menu. A shared settings block (music + SFX sliders, vibration and quality cycle-buttons) lives in both the main menu and the pause menu; changes apply live (SFX drives the master `AudioListener` volume today; music volume is stored for the Phase 5 audio service) and persist to the save immediately. The pause menu never opens over the level-up, death, or victory freezes.
